# Pricing Configuration

## Monetization Model: Subscription (IAP)

## Subscription Group
- **Group Name**: Pipely Pro
- **Group ID**: PipelyPro

## Subscription Tiers

### 1. Monthly Subscription
- **Reference Name**: Monthly Pro
- **Product ID**: `com.zzoutuo.PipelyCRM.monthly`
- **Price**: $9.99 per month
- **Display Name**: Pipely Pro Monthly
- **Description**: Unlimited contacts, deals, and team
- **Localization**: English (US)

### 2. Yearly Subscription
- **Reference Name**: Yearly Pro
- **Product ID**: `com.zzoutuo.PipelyCRM.yearly`
- **Price**: $79.99 per year (33% savings vs monthly, equivalent to $6.67/month)
- **Display Name**: Pipely Pro Yearly
- **Description**: Best value - save 33% annually
- **Localization**: English (US)

## Free Tier
- **Price**: Free (No charge)
- **Limits**:
  - Up to 50 contacts
  - 1 pipeline (5 default stages)
  - Up to 20 active deals
  - 3 follow-up reminders per day
  - Basic reports (pipeline total value)
  - 1 user
  - Dark mode

## Pro Tier (Monthly or Yearly)
- **Features**:
  - Unlimited contacts
  - Unlimited deals
  - Unlimited pipelines
  - Unlimited follow-up reminders
  - Up to 5 team members
  - Advanced reports (conversion rates, team performance)
  - Email templates
  - Data export CSV
  - Custom pipeline stages
  - Custom tags and fields
  - Priority support (yearly only)

## Free Trial
- **Duration**: 7 days
- **Type**: Free trial (auto-converts to paid monthly)

## App Store Connect Pricing
- **Price Tier**: Free (app download)
- **Subscription Pricing**: Set in App Store Connect
  - Monthly: Tier 89 ($9.99)
  - Yearly: Tier 799 ($79.99)

## Policy Pages Required
- Support Page: ✅ (Must include subscription management info)
- Privacy Policy: ✅
- Terms of Use: ✅ (REQUIRED for subscription apps)

## Apple IAP Compliance Checklist
- [ ] Auto-renewal terms included in Terms
- [ ] Cancellation instructions included
- [ ] Pricing clearly stated
- [ ] Free trial terms included (if applicable)
- [ ] Restore purchases functionality implemented

## Competitive Pricing Comparison
| App | 5-person team/month | Free tier | Per-user pricing |
|-----|-------------------|-----------|-----------------|
| **Pipely** | **$9.99** | ✅ (50 contacts) | ❌ Flat rate |
| HubSpot | $75 | ✅ (2 users) | ✅ $15/user |
| Pipedrive | $70 | ❌ | ✅ $14/user |
| Less Annoying CRM | $75 | ❌ | ✅ $15/user |
| OnePageCRM | $75 | ❌ | ✅ $15/user |
